...
def _branch_exists(branch_name: str, include_remote: bool = True) -> bool:
	"""
...
class WorkflowStrategy:
	...
	def get_pr_templates(self, branch_type: str) -> dict[str, str]:
		...
		"""
		# Default implementation provides basic templates
		return {
	...
	def get_branch_metadata(self, branch_name: str) -> dict[str, Any]:
		...
		try:
			...
			date_cmd = [
				...
				"--format=%ad",
				"--date=relative",
				branch_name if _branch_exists(branch_name) else f"origin/{branch_name}",
			]
...
""",
		...
		return templates.get(branch_type, super().get_pr_templates(branch_type))
