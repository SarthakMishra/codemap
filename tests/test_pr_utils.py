...
def test_get_current_branch(mock_run_git_command):
...
def test_get_current_branch_error(mock_run_git_command):
...
def test_get_default_branch_from_remote(mock_run_git_command):
...
def test_get_default_branch_fallback_to_main(mock_run_git_command):
    """Test get_default_branch function when falling back to main."""
    mock_run_git_command.side_effect = [
        "Remote info without HEAD branch",
        "origin/main\norigin/feature-branch"
    ]
...
def test_get_default_branch_fallback_to_master(mock_run_git_command):
...
def test_create_branch(mock_run_git_command):
...
def test_create_branch_error(mock_run_git_command):
...
def test_checkout_branch(mock_run_git_command):
...
def test_checkout_branch_error(mock_run_git_command):
...
def test_branch_exists_local(mock_run_git_command):
...
def test_branch_exists_remote(mock_run_git_command):
...
def test_branch_not_exists(mock_run_git_command):
...
def test_branch_exists_git_error(mock_run_git_command):
...
def test_push_branch(mock_run_git_command):
...
def test_push_branch_force(mock_run_git_command):
...
def test_push_branch_error(mock_run_git_command):
...
def test_get_commit_messages(mock_run_git_command):
...
def test_get_commit_messages_empty(mock_run_git_command):
...
def test_get_commit_messages_error(mock_run_git_command):
...
def test_generate_pr_title_from_commits_empty():
...
def test_generate_pr_title_from_commits_feat():
...
def test_generate_pr_title_from_commits_fix():
...
def test_generate_pr_title_from_commits_with_scope():
...
def test_generate_pr_title_from_commits_fallback():
...
def test_generate_pr_description_from_commits_empty():
...
def test_generate_pr_description_from_commits():
...
def test_create_pull_request(mock_subprocess_run):
    ...
    mock_subprocess_run.side_effect = [mock_version_process, mock_create_process]
    ...
    create_call_args = mock_subprocess_run.call_args_list[1][0][0]
...
def test_create_pull_request_gh_not_installed(mock_subprocess_run):
...
def test_create_pull_request_error(mock_subprocess_run):
    """Test create_pull_request function when gh command fails."""
    # First mock for gh --version check
    mock_version_process = MagicMock()
    mock_version_process.returncode = 0
    
    # Second mock for gh pr create that fails
    mock_create_process = MagicMock()
    mock_create_process.stderr = "Error creating PR"
    mock_create_process.returncode = 1
    mock_create_process.check = True
...
def test_update_pull_request(mock_subprocess_run):
    """Test update_pull_request function."""
    # First mock for gh --version check
    mock_version_process = MagicMock()
    mock_version_process.returncode = 0
    
    # Second mock for gh pr edit
    mock_edit_process = MagicMock()
    mock_edit_process.returncode = 0
    mock_edit_process.stdout = ""
    
    # Third mock for gh pr view
    ...
    mock_subprocess_run.side_effect = [mock_version_process, mock_edit_process, mock_view_process]
    ...
    with patch("codemap.utils.pr_utils.get_current_branch") as mock_get_current_branch:
        ...
        edit_call_args = mock_subprocess_run.call_args_list[1][0][0]
        assert edit_call_args[0:3] == ["gh", "pr", "edit"]
        assert edit_call_args[3] == "123"
        
        # Third call should be to get the PR URL
        view_call_args = mock_subprocess_run.call_args_list[2][0][0]
...
def test_update_pull_request_gh_not_installed(mock_subprocess_run):
...
def test_update_pull_request_error(mock_subprocess_run):
    """Test update_pull_request function when gh command fails."""
    # First mock for gh --version check
    mock_version_process = MagicMock()
    mock_version_process.returncode = 0
    
    # Mock for get_current_branch
    with patch("codemap.utils.pr_utils.get_current_branch") as mock_get_current_branch:
        mock_get_current_branch.return_value = "feature-branch"
        
        # Second mock for gh pr edit that fails
        mock_subprocess_run.side_effect = [
            mock_version_process, 
...
def test_get_existing_pr(mock_subprocess_run):
    """Test get_existing_pr function."""
    # First mock for gh --version check
    mock_version_process = MagicMock()
    mock_version_process.returncode = 0
    
    # Second mock for gh pr list
    ...
    mock_subprocess_run.side_effect = [mock_version_process, mock_list_process]
    ...
    list_call_args = mock_subprocess_run.call_args_list[1][0][0]
...
def test_get_existing_pr_not_found(mock_subprocess_run):
    """Test get_existing_pr function when PR doesn't exist."""
    # First mock for gh --version check
    mock_version_process = MagicMock()
    mock_version_process.returncode = 0
    
    # Second mock for gh pr list with no results
    mock_list_process = MagicMock()
...
def test_suggest_branch_name_empty():
...
def test_suggest_branch_name_non_conventional():
