...
@pytest.mark.usefixtures("mock_git_utils")
def test_pr_create_command() -> None:
    """Test the PR create command."""
    runner = CliRunner()

    # Run the command (we're checking the typer setup + basic CLI validation)
    result = runner.invoke(
        app,
        ["pr", "create", "--branch", "feature-branch", "--non-interactive"],
    )

    # Check only that the command exits successfully
