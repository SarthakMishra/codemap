"""Tests for the CommitCommand class in git/commit_generator/command.py."""
...
class TestCommitCommandChanges(GitTestBase):
	...
	def setup_method(self) -> None:
		...
		with (
			...
			patch(
				"codemap.git.commit_generator.command.CommitMessageGenerator", return_value=self.mock_message_generator
			),
		):
	...
	def test_get_changes_success(self) -> None:
		...
		with (
			patch("codemap.git.commit_generator.command.run_git_command") as mock_run_git,
			patch("codemap.git.commit_generator.command.get_staged_diff") as mock_staged,
			patch("codemap.git.commit_generator.command.get_unstaged_diff") as mock_unstaged,
			patch("codemap.git.commit_generator.command.get_untracked_files") as mock_untracked,
		):
			# Configure mocks
	...
	def test_get_changes_with_staging_error(self) -> None:
		...
		with (
			...
			patch("codemap.git.commit_generator.command.get_unstaged_diff") as mock_unstaged,
			patch("codemap.git.commit_generator.command.get_untracked_files") as mock_untracked,
			patch("codemap.git.commit_generator.command.logger") as mock_logger,
		):
			# Configure mocks
...
class TestCommitCommandMessageGeneration(GitTestBase):
	...
	def test_generate_commit_message_success(self) -> None:
		...
		with patch("codemap.git.commit_generator.command.logger") as mock_logger:
			# Configure the mock to return a message and True for LLM-generated
			self.mock_message_generator.generate_message.return_value = ("feat: Test commit message", True)
	...
	def test_generate_commit_message_fallback(self) -> None:
		...
		with patch("codemap.git.commit_generator.command.logger") as mock_logger:
			...
			self.mock_message_generator.generate_message.return_value = ("chore: Update files", False)
	...
	def test_generate_commit_message_llm_error(self) -> None:
		...
		with patch("codemap.git.commit_generator.command.logger") as mock_logger:
			...
			self.mock_message_generator.generate_message.side_effect = LLMError("Failed to generate message with LLM")
	...
	def test_generate_commit_message_with_existing_description(self) -> None:
		...
		# Arrange: Set up mocks
		with patch("codemap.git.commit_generator.command.logger"):
	...
	def test_generate_commit_message_other_error(self) -> None:
		...
		with patch("codemap.git.commit_generator.command.logger") as mock_logger:
			...
			self.mock_message_generator.generate_message.side_effect = ValueError(
				"Invalid input for message generation"
			)
...
class TestCommitCommandRun(GitTestBase):
	...
	This tests the main entry point method that drives the commit workflow.
