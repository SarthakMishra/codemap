...
class TestCommitCommandChanges(GitTestBase):
	...
	def setup_method(self) -> None:
		...
		with (
			patch("codemap.git.command.CommitUI", return_value=self.mock_ui),
			patch("codemap.git.command.DiffSplitter", return_value=self.mock_splitter),
	...
	def test_get_changes_success(self) -> None:
		...
		with (
			...
			patch("codemap.git.command.get_unstaged_diff") as mock_unstaged,
			patch("codemap.git.command.get_untracked_files") as mock_untracked,
		):
			# Configure mocks
	...
	def test_get_changes_with_staging_error(self) -> None:
		...
		with (
			...
			patch("codemap.git.command.get_unstaged_diff") as mock_unstaged,
			patch("codemap.git.command.get_untracked_files") as mock_untracked,
			patch("codemap.git.command.logger") as mock_logger,
		):
			# Configure mocks
...
class TestCommitCommandMessageGeneration(GitTestBase):
	...
	def test_generate_commit_message_success(self) -> None:
		...
		with (
			patch("codemap.git.command.generate_message") as mock_generate,
			patch("codemap.git.command.logger") as mock_logger,
		):
