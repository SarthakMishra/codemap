...
def test_python_chunking(python_code: str) -> None:
    """Test chunking Python code."""
    chunker = SyntaxChunker()
    chunks = chunker.chunk(python_code, Path("test.py"))
...
def test_chunk_splitting(python_code: str) -> None:
    ...
    # Split the chunk at a small size to force multiple chunks
    max_size = 400  # Adjust size to be larger than the current chunk sizes
    split_chunks = chunker.split(long_func, max_size)
    ...
    # There might be some overhead due to implementation details
