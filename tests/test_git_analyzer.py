...
def git_analyzer(mock_subprocess_run: MagicMock) -> GitMetadataAnalyzer:
	...
	with (
		patch("codemap.processor.analysis.git.analyzer.GitMetadataAnalyzer._verify_git_repo"),
		patch("codemap.processor.analysis.git.analyzer.Path") as mock_path,
	):
...
class TestGitRepoDetection:
	"""Test cases for Git repository detection and validation."""

	def test_detect_repo_root(self, mock_subprocess_run: MagicMock) -> None:
	...
	def test_detect_repo_root_error(self, mock_subprocess_run: MagicMock) -> None:
	...
	def test_verify_git_repo(self) -> None:
...
@pytest.mark.git
@pytest.mark.unit
class TestGitBranchAndCommit:
	"""Test cases for Git branch and commit information retrieval."""

	def test_get_current_branch(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
	...
	def test_get_current_branch_error(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
	...
	def test_get_current_commit(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
	...
	def test_get_current_commit_error(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
...
class TestGitBlameInfo:
	"""Test cases for Git blame information retrieval."""

	def _setup_blame_output(self) -> str:
		"""Setup mock blame output data."""
		return (
	...
	def test_get_blame_info(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
		"""Test getting git blame information for a range of lines."""
		# Create a sample git blame porcelain output
		mock_subprocess_run.return_value.stdout = self._setup_blame_output()
	...
	def test_get_blame_info_error(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
...
class TestGitMetadata:
	"""Test cases for Git metadata creation and handling."""

	def _create_sample_blame_info(self) -> list[GitBlameInfo]:
		"""Create sample GitBlameInfo objects for testing."""
		return [
			GitBlameInfo(
	...
	def _create_multiple_blame_info(self) -> list[GitBlameInfo]:
		"""Create multiple GitBlameInfo objects with different timestamps."""
		...
		return [older_blame, newer_blame]

	def test_create_git_metadata(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
		...
		blame_info = self._create_sample_blame_info()
	...
	def test_create_git_metadata_with_multiple_commits(
		self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock
	) -> None:
		...
		blame_info = self._create_multiple_blame_info()
	...
	def test_create_git_metadata_error(self, git_analyzer: GitMetadataAnalyzer, mock_subprocess_run: MagicMock) -> None:
...
class TestChunkEnrichment:
	"""Test cases for chunk enrichment with Git metadata."""

	def _create_test_chunk(self) -> tuple[Chunk, Location]:
		"""Create a test chunk with location for testing."""
		location = Location(file_path=Path("/fake/repo/path/src/file.py"), start_line=1, end_line=2)
	...
	def _create_sample_git_metadata(self) -> GitMetadata:
		"""Create a sample GitMetadata object for testing."""
		return GitMetadata(
			is_committed=True,
			commit_id="abcdef1234567890abcdef1234567890abcdef12",
	...
	def test_enrich_chunk(self, git_analyzer: GitMetadataAnalyzer) -> None:
		"""Test enriching a chunk with Git metadata."""
		# Create a mock chunk with location
		chunk, location = self._create_test_chunk()

		# Mock create_git_metadata to return a predefined GitMetadata
		git_metadata = self._create_sample_git_metadata()
	...
	def test_enrich_chunk_recursive(self, git_analyzer: GitMetadataAnalyzer) -> None:
	...
	def test_enrich_chunk_without_location(self, git_analyzer: GitMetadataAnalyzer) -> None:
