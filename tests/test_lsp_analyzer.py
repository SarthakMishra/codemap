...
class TestLSPAnalyzer:
	...
	@pytest.fixture
	def mock_lsp_server(self) -> MagicMock:
	...
	@pytest.fixture
	def lsp_analyzer(self, sample_repo_path: Path, mock_lsp_server: MagicMock) -> Generator[LSPAnalyzer, None, None]:
		"""Create an LSP analyzer instance for testing."""
		with patch("multilspy.SyncLanguageServer.create", return_value=mock_lsp_server):
	...
	@pytest.fixture
	def method_chunk(self, user_class_chunk: Chunk) -> Chunk:
		...
		return Chunk(content=method_content, metadata=method_metadata, parent=user_class_chunk)

	@pytest.fixture
	def user_class_with_method(self, user_class_chunk: Chunk, method_chunk: Chunk) -> Chunk:
		"""Create a user class chunk with a method child."""
		return Chunk(content=user_class_chunk.content, metadata=user_class_chunk.metadata, children=[method_chunk])
	...
	class TestLanguageServerManagement:
		"""Tests for language server creation and management."""
	...
	class TestPathAndPositionHandling:
		"""Tests for path normalization and position handling."""
	...
	class TestChunkAnalysis:
		"""Tests for chunk analysis functionality."""

		def _configure_mock_lsp_server(self, mock_server: MagicMock) -> None:
			"""Configure the mock LSP server with standard responses."""
		...
		def test_analyze_chunk(
			...
			self._configure_mock_lsp_server(mock_server)

			# Create a patched _get_language_server that returns our mock
			with (
				patch.object(lsp_analyzer, "_get_language_server", return_value=mock_server),
				patch.object(mock_server, "start_server"),
			):
				# Run the analysis
		...
		def test_enrich_chunks(self, lsp_analyzer: LSPAnalyzer, user_class_with_method: Chunk) -> None:
			"""Test enriching multiple chunks with LSP metadata."""
			# Create sample LSP metadata for testing
