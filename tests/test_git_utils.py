...
class TestGitUtils:
	...
	def test_get_repo_root_failure(self) -> None:
		...
		with (
			patch("codemap.utils.git_utils.run_git_command", side_effect=GitError("Not in a Git repository")),
	...
	def test_stage_files_existing_files(self) -> None:
		...
		with (
			patch("codemap.utils.git_utils.run_git_command") as mock_run_cmd,
	...
	def test_stash_staged_changes_with_other_files(self) -> None:
		...
		with (
			patch("codemap.utils.git_utils.get_other_staged_files", return_value=["file2.py"]),
			patch("codemap.utils.git_utils.run_git_command"),
		):
			result = stash_staged_changes(["file1.py"])
	...
	def test_unstage_files_failure(self) -> None:
		...
		with (
			patch("codemap.utils.git_utils.run_git_command", side_effect=GitError("Failed to unstage")),
	...
	def test_commit_only_files(self) -> None:
		...
		with (
			...
			patch("codemap.utils.git_utils.stage_files") as mock_stage,
			patch("codemap.utils.git_utils.get_other_staged_files", return_value=["other.py"]),
		):
	...
	def test_commit_only_files_with_hooks_disabled(self) -> None:
		...
		with (
			patch("subprocess.run") as mock_run,
			patch("codemap.utils.git_utils.stage_files"),
			patch("codemap.utils.git_utils.get_other_staged_files", return_value=[]),
		):
			# Mock subprocess.run for status and commit
			mock_run.side_effect = [
