...
class TestGitUtils:
    ...
    def test_get_repo_root_success(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command", return_value="/path/to/repo\n"):
    ...
    def test_get_repo_root_failure(self) -> None:
        ...
        with patch(
            "codemap.utils.git_utils.run_git_command", side_effect=GitError("Not in a Git repository")
    ...
    def test_validate_repo_path_success(self) -> None:
        ...
        with patch("codemap.utils.git_utils.get_repo_root", return_value=Path("/path/to/repo")):
    ...
    def test_validate_repo_path_failure(self) -> None:
        ...
        with patch("codemap.utils.git_utils.get_repo_root", side_effect=GitError("Not in a Git repository")):
            result = validate_repo_path(Path("/some/path"))
            assert result is None

    def test_get_staged_diff(self) -> None:
        """Test getting staged diff."""
        with patch("codemap.utils.git_utils.run_git_command") as mock_run_cmd:
    ...
    def test_stage_files_empty_list(self) -> None:
        ...
        with patch("codemap.utils.git_utils.logger") as mock_logger:
            stage_files([])
    ...
    def test_stage_files_existing_files(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command") as mock_run_cmd, patch(
    ...
    def test_stage_files_invalid_filenames(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command") as mock_run_cmd, patch(
            "codemap.utils.git_utils.logger"
        ) as mock_logger:
            mock_run_cmd.side_effect = [
                "",  # git status
                "",  # tracked files
    ...
    def test_stage_files_untracked_nonexistent(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command") as mock_run_cmd, patch(
            "pathlib.Path.exists", return_value=False
        ), patch("codemap.utils.git_utils.logger") as mock_logger:
            mock_run_cmd.side_effect = [
                "",  # git status
                "",  # tracked files (empty)
                "",  # index files (empty)
    ...
    def test_get_other_staged_files(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command", return_value="file1.py\nfile2.py\nfile3.py"):
    ...
    def test_stash_staged_changes_no_other_files(self) -> None:
        ...
        with patch("codemap.utils.git_utils.get_other_staged_files", return_value=[]):
    ...
    def test_stash_staged_changes_with_other_files(self) -> None:
        ...
        with patch("codemap.utils.git_utils.get_other_staged_files", return_value=["file2.py"]), patch(
    ...
    def test_unstash_changes_no_stash(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command", return_value=""):
    ...
    def test_unstage_files_failure(self) -> None:
        ...
        with patch("codemap.utils.git_utils.run_git_command", side_effect=GitError("Failed to unstage")), pytest.raises(
            GitError, match="Failed to unstage files: file1.py, file2.py"
        ):
            unstage_files(["file1.py", "file2.py"])
    ...
    def test_commit_only_files(self) -> None:
        ...
        with patch("subprocess.run") as mock_run, patch("codemap.utils.git_utils.stage_files") as mock_stage, patch(
            "codemap.utils.git_utils.get_other_staged_files", return_value=["other.py"]
        ):
    ...
    def test_commit_only_files_with_hooks_disabled(self) -> None:
        ...
        with patch("subprocess.run") as mock_run, patch("codemap.utils.git_utils.stage_files"), patch(
            "codemap.utils.git_utils.get_other_staged_files", return_value=[]
        ):
            # Mock subprocess.run for status and commit
            mock_run.side_effect = [
