...
class TestProcessFile:
    ...
    def test_process_file_with_exception(self, processor: DocumentationProcessor) -> None:
        ...
        with patch(
            "codemap.analyzer.processor.count_tokens", side_effect=UnicodeDecodeError("utf-8", b"", 0, 1, "test error")
    ...
    def test_process_file_os_error(self, processor: DocumentationProcessor) -> None:
        ...
        with patch("codemap.analyzer.processor.count_tokens", side_effect=OSError("test error")), patch.object(
            logging.getLogger("codemap.analyzer.processor"), "warning"
        ) as mock_warning:
            file_path = Path("test_file.py")
            file_info, new_total = processor.process_file(file_path)
...
class TestProcessDirectory:
    ...
    def test_process_directory_with_progress(self, processor: DocumentationProcessor) -> None:
        ...
        with patch("os.walk", return_value=mock_walk_data), patch(
            "codemap.analyzer.processor.Progress"
