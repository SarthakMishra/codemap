...
class TestSetupMessageGenerator:
    ...
    def test_setup_message_generator(self, commit_options: CommitOptions) -> None:
        ...
        with patch("codemap.cli.commit_cmd.create_universal_generator") as mock_create:
            mock_create.return_value = MagicMock(spec=MessageGenerator)
...
class TestGenerateCommitMessage:
    ...
    def test_generate_commit_message_smart_mode(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        mock_generator = MagicMock(spec=MessageGenerator)
    ...
    def test_generate_commit_message_simple_mode(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        with patch("codemap.cli.commit_cmd.logger"), patch(
            "codemap.utils.llm_utils.generate_message"
        ) as mock_gen_message:
            # Mock the underlying generate_message function
            mock_gen_message.return_value = ("feat: Update file1.py", False)

            message, used_llm = generate_commit_message(mock_diff_chunk, mock_generator, GenerationMode.SIMPLE)

            # Check that the message matches what the mock returned
            assert message == "feat: Update file1.py"
    ...
    def test_generate_commit_message_error(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        mock_generator = MagicMock(spec=MessageGenerator)
        mock_generator.fallback_generation.return_value = "test: Update file1.py and file2.py"

        # Set up mock_diff_chunk with required properties
        mock_diff_chunk.files = ["file1.py", "file2.py"]
        mock_diff_chunk.content = "test diff content"

        with patch("codemap.cli.commit_cmd.console") as mock_console, patch("codemap.cli.commit_cmd.logger"), patch(
            "codemap.utils.llm_utils.generate_message"
        ) as mock_gen_message:
            ...
            mock_gen_message.side_effect = LLMError("Test error")

            message, used_llm = generate_commit_message(mock_diff_chunk, mock_generator, GenerationMode.SMART)

            assert message == "test: Update file1.py and file2.py"
            assert used_llm is False
            # Verify console.print was called to report the error
            mock_console.print.assert_called_once()
...
class TestCheckOtherFiles:
    ...
    def test_check_other_files_none(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.get_other_staged_files", return_value=[]), patch(
            "codemap.cli.commit_cmd.get_untracked_files", return_value=[]
        ):
    ...
    def test_check_other_files_staged(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.get_other_staged_files", return_value=["file3.py"]), patch(
            "codemap.cli.commit_cmd.get_untracked_files", return_value=[]
        ):
            chunk_files = ["file1.py", "file2.py"]
            other_staged, other_untracked, has_other = _check_other_files(chunk_files)
    ...
    def test_check_other_files_untracked(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.get_other_staged_files", return_value=[]), patch(
            "codemap.cli.commit_cmd.get_untracked_files", return_value=["file3.py"]
        ):
            chunk_files = ["file1.py", "file2.py"]
            other_staged, other_untracked, has_other = _check_other_files(chunk_files)
    ...
    def test_check_other_files_both(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.get_other_staged_files", return_value=["file3.py"]), patch(
            "codemap.cli.commit_cmd.get_untracked_files", return_value=["file4.py"]
        ):
            chunk_files = ["file1.py", "file2.py"]
            other_staged, other_untracked, has_other = _check_other_files(chunk_files)
...
class TestCommitChanges(GitTestBase):
    ...
    def test_commit_changes_success(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.Path") as mock_path, patch(
            "codemap.cli.commit_cmd.commit_only_files"
        ) as mock_commit_only:
    ...
    def test_commit_changes_failure(self) -> None:
        """Test failed commit."""
        with patch("codemap.cli.commit_cmd.Path") as mock_path, patch(
            "codemap.cli.commit_cmd.commit_only_files", side_effect=Exception("Commit failed")
        ), patch("codemap.cli.commit_cmd.console") as mock_console, patch(
            "codemap.cli.commit_cmd.logger"
        ) as mock_logger:
            # Configure Path mock
            mock_path.return_value.exists.return_value = True
...
class TestCommitWithMessage:
    ...
    def test_commit_with_message(self, mock_diff_chunk: DiffChunk) -> None:
        """Test commit with message."""
        with patch("codemap.cli.commit_cmd._perform_commit") as mock_perform, patch(
            "codemap.cli.commit_cmd.console"
        ) as mock_console:
...
class TestCommitWithUserInput:
    ...
    def test_commit_with_user_input_accept(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        with patch("codemap.cli.commit_cmd.console"), patch("questionary.select") as mock_select, patch(
            "codemap.cli.commit_cmd._perform_commit"
        ) as mock_perform, patch("codemap.cli.commit_cmd._edit_commit_message") as mock_edit:
            # Configure the mocks
    ...
    def test_commit_with_user_input_edit(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        with patch("codemap.cli.commit_cmd.console"), patch("questionary.select") as mock_select, patch(
            "codemap.cli.commit_cmd._edit_commit_message"
        ) as mock_edit, patch("codemap.cli.commit_cmd._perform_commit") as mock_perform:
    ...
    def test_commit_with_user_input_skip(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        with patch("codemap.cli.commit_cmd.console") as mock_console, patch(
            "codemap.cli.commit_cmd.print_chunk_summary"
        ), patch("codemap.cli.commit_cmd.generate_commit_message") as mock_generate, patch(
            "questionary.select"
        ) as mock_select:
            # Configure console mock to ensure it's properly tracked
            mock_console.print = MagicMock()
...
class TestProcessChunkInteractively:
    ...
    def test_process_chunk_interactive_with_llm(self, mock_diff_chunk: DiffChunk) -> None:
        ...
        with patch("codemap.cli.commit_cmd.print_chunk_summary"), patch(
            "codemap.cli.commit_cmd.generate_commit_message"
        ) as mock_generate, patch("codemap.cli.commit_cmd.loading_spinner"), patch(
            "questionary.select"
        ) as mock_select, patch("codemap.cli.commit_cmd._commit_with_message") as mock_commit_with_message:
...
class TestDisplaySuggestedMessages:
    ...
    def test_display_suggested_messages(self, commit_options: CommitOptions) -> None:
        ...
        with patch("codemap.cli.commit_cmd.console") as mock_console, patch(
            "codemap.cli.commit_cmd.generate_commit_message"
        ) as mock_generate, patch("codemap.cli.commit_cmd.print_chunk_summary"):
...
def test_commit_changes_no_valid_files() -> None:
    ...
    with patch("codemap.cli.commit_cmd.run_git_command") as mock_run_git, patch(
        "codemap.cli.commit_cmd.Path.exists", return_value=False
    ), patch("codemap.cli.commit_cmd.console") as mock_console, patch("codemap.cli.commit_cmd.logger") as mock_logger:
...
def test_commit_changes_exception() -> None:
    ...
    with patch("codemap.cli.commit_cmd.run_git_command") as mock_run_git, patch(
        "codemap.cli.commit_cmd.Path.exists", return_value=True
    ), patch("codemap.cli.commit_cmd.commit_only_files") as mock_commit, patch(
        "codemap.cli.commit_cmd.console"
    ) as mock_console, patch("codemap.cli.commit_cmd.logger") as mock_logger:
        # Mock tracked files
...
def test_perform_commit_success() -> None:
    ...
    with patch("codemap.cli.commit_cmd._commit_changes", return_value=True) as mock_commit, patch(
        "codemap.cli.commit_cmd.console"
    ) as mock_console:
        # Call function
...
class TestCommitCommand(CLITestBase, GitTestBase):
    ...
    def test_commit_command_initialization(self) -> None:
        ...
        with patch("codemap.cli.commit_cmd.validate_repo_path", return_value=Path("/fake/repo")), patch(
            "codemap.cli.commit_cmd.setup_message_generator"
        ), patch("codemap.cli.commit_cmd.console"), patch(
            "codemap.cli.commit_cmd.get_staged_diff"
        ) as mock_staged, patch("codemap.cli.commit_cmd.get_unstaged_diff") as mock_unstaged, patch(
            "codemap.cli.commit_cmd.get_untracked_files"
        ) as mock_untracked:
