...
class TestPipeline:
    ...
    def pipeline(
        ...
        with patch("codemap.processor.pipeline.GitMetadataAnalyzer", return_value=mock_git_analyzer), patch(
            "codemap.processor.pipeline.LSPAnalyzer", return_value=mock_lsp_analyzer
        ), patch("codemap.processor.pipeline.TreeSitterChunker", return_value=mock_chunker), patch(
            "codemap.processor.pipeline.EmbeddingGenerator", return_value=mock_embedding_generator
        ), patch("codemap.processor.pipeline.LanceDBStorage", return_value=mock_storage), patch(
            "codemap.processor.pipeline.FileWatcher", return_value=mock_file_watcher
        ):
